...
export function ProprietaryFramework({
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-charcoal via-charcoal to-charcoal">
              ...
              <div key={index} className="relative">
                  ...
                  <div className="space-y-2">
                    {phase.outcomes.map((outcome, idx) => (
                      <div key={idx} className="flex items-start gap-2">
                        <CheckCircle2 className="w-3 h-3 sm:w-4 sm:h-4 text-amber flex-shrink-0 mt-0.5" />
                ...
                {index < phases.length - 1 && (
                  <div className="hidden lg:flex absolute top-1/2 -right-4 transform -translate-y-1/2 z-20">
                    <ArrowRight className="w-8 h-8 text-amber" />
                  </div>
                )}
              </div>
            ))}
