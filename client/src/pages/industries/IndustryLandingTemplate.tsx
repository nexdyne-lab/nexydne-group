...
export default function IndustryLandingTemplate({
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-[#051C2C] via-[#0a3d5c] to-[#051C2C] text-white py-16 sm:py-20 lg:py-24 overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
            {stats.map((stat, index) => (
              <div key={index} className="text-center">
                <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#051C2C] mb-1 sm:mb-2">
                  {stat.value}
                </div>
      ...
      <section id="case-studies" className="py-12 sm:py-16 lg:py-20">
        <div className="container">
          <div className="flex items-center justify-between mb-12">
            ...
            <Link
              href="/case-studies"
              className="hidden md:inline-flex items-center gap-2 text-[#0077B5] font-semibold hover:underline"
            >
              ...
              >
                <div className="p-6">
                  <div className="flex items-baseline gap-2 mb-4">
                    ...
                    <span className="text-sm text-slate-500">{study.metricLabel}</span>
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 group-hover:text-[#0077B5] transition-colors mb-2">
                    {study.title}
                  </h3>
                  <p className="text-slate-600 text-sm">{study.description}</p>
                </div>
                <div className="px-6 py-4 bg-slate-50 border-t border-slate-100">
                  <span className="inline-flex items-center gap-2 text-sm font-semibold text-[#0077B5]">
                    Read Case Study
          ...
          <div className="mt-8 md:hidden text-center">
            <Link
              href="/case-studies"
              className="inline-flex items-center gap-2 text-[#0077B5] font-semibold"
            >
              View All Case Studies
              <ArrowRight className="w-4 h-4" />
            </Link>
                ...
                {resources.map((resource, index) => {
                  ...
                  return (
                    <Link
                      key={index}
                      href={resource.href}
                      className="group flex items-start gap-4 bg-white p-6 rounded-xl border border-slate-200 hover:border-[#0077B5] hover:shadow-md transition-all"
                    >
                      <div className="flex-shrink-0 w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center group-hover:bg-[#0077B5]/10 transition-colors">
                        <TypeIcon className="w-5 h-5 text-slate-600 group-hover:text-[#0077B5] transition-colors" />
                      </div>
                      <div className="flex-1">
                        <span className="text-xs text-slate-500 uppercase tracking-wider">
                          {resource.type}
                        </span>
                        <h3 className="text-lg font-bold text-slate-900 group-hover:text-[#0077B5] transition-colors">
                          {resource.title}
                        </h3>
                      ...
                      <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-[#0077B5] group-hover:translate-x-1 transition-all flex-shrink-0" />
                    </Link>
                  );
                })}
              </div>
      ...
      {/* CTA Section */}
      <section className="py-12 sm:py-16 lg:py-20 bg-[#051C2C] text-white">
        <div className="container">
